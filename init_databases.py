...
================================================================
Creates isolated SQLite databases for 20 languages inside a single
./database/ folder in the Current Working Directory (CWD).

Folder Structure Generated:
  ./database/en.db  ./database/es.db  ./database/de.db  ...  ./database/iu.db
"""
...
# ═══════════════════════════════════════════════════════════════════
# SECTION 2: Seed Dictionaries (20 Languages)
# ═══════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════
# SECTION 3: Schema SQL (Shared — Same as app.py)
# ═══════════════════════════════════════════════════════════════════
...
class DatabaseInitializer:
    """Creates and seeds a language-specific SQLite database in ./database/."""
    ...
    def _seed_data(self):
        ...
        if not words:
            logger.warning(f"No seed data for {self.lang_code}. Skipping.")
            return

        cursor = self.conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM dictionary")
        if cursor.fetchone()[0] > 0:
            logger.info("Already populated. Seeding skipped.")
            return

        logger.info(f"Seeding {len(words)} tokens …")
        ...
        try:
            ...
            logger.info(
                f"Seeded {len(freq)} words, "
                f"{len(bigram_freq)} bigrams, "
                f"{len(trigram_freq)} trigrams."
            )
        except sqlite3.Error as e:
...
def main():
    logger.info(f"Initializing databases for {len(LANGUAGES)} languages.")
    logger.info(f"Target directory: {os.path.join(os.getcwd(), DB_FOLDER)}\n")

    for lang in LANGUAGES:
        try:
            DatabaseInitializer(lang).init_database()
        except Exception as e:
            logger.error(f"Halting {lang}: {e}")
